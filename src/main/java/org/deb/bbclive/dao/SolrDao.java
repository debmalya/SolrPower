...
/**
 * While the UpdateResponse and QueryResponse interfaces that SolrJ provides are
 * useful, it is often more convenient to work with domain-specific objects that
 * can more easily be understood by your application.
 */
...
public class SolrDao {
	...
	/**
	 * This will add news extract to SOLR. Under production circumstances,
	 * documents should be indexed in larger batches, instead of one at a time.
	 *
	 * @param news
	 *            extract.
	 * @param entities
	...
	public UpdateResponse addLotsOfNews(List<NewsExtract> newsList) throws IOException, SolrServerException {
		...
		UpdateResponse updateResponse = solrClient.addBeans(appContext.getCollectionName(), newsList);
		try {
			// Indexed documents must be committed
			solrClient.commit(appContext.getCollectionName());
//			solrClient.optimize();
		} catch(IOException | SolrServerException ex){
			LOGGER.error("ERR from SolrDao :" + ex.getMessage(),ex);
